...
function saveParking(lng, lat) {
  const data = { lng, lat, time: Date.now() };
...
function promptSaveParking() {
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      ...
      saveParking(longitude, latitude);
...
function renderOverview(query) {
  ...
  const favLocations = favIds.map(id => appState.locations.find(l => String(l.id) === id)).filter(Boolean);
  {
    ...
    expandBtn.addEventListener('click', () => {
      appState.groupExpanded.set('favorites', !expanded);
      renderOverview(query);
    });
